...
export class Edu0103Service {
  ...
  search(request: any): Observable<any> {
    ...
    const url = ApiUrlUtil.buildQueryString(
      environment.apiURL + '/edu/edu0103/search',
      params
    );
  ...
  save(request: any): Observable<any> {
    const headers: HttpHeaders = HeadersUtil.getHeadersAuth();
    const url = environment.apiURL + '/edu/edu0103/save';
    return this.http.post<any>(url, request, { headers: headers });
  }

  deleteListCheck(request: any): Observable<any> {
    const headers: HttpHeaders = HeadersUtil.getHeadersAuth();
    const url = environment.apiURL + '/edu/edu0103/deleteListCheck'
    return this.http.post<any>(url, request, { headers: headers });
  }
