...
@Injectable()
export class Edu0203Service {

  constructor(private http: HttpClient) { }

  save(request: any): Observable<any> {
    const headers: HttpHeaders = HeadersUtil.getHeadersAuth();
    const url = environment.apiURL + '/edu/edu0203/save';
  ...
  search(request: any): Observable<any> {
    ...
    const url = ApiUrlUtil.buildQueryString(
      environment.apiURL + '/edu/edu0203/search',
      params
    );
  ...
  delete(eventId: String): Observable<any> {
    const headers: HttpHeaders = HeadersUtil.getHeadersAuth();
    const url = environment.apiURL + '/edu/edu0203/delete/' + eventId;
    return this.http.delete<any>(url, { headers: headers });
  }

  deleteListCheck(request: any): Observable<any> {
    const headers: HttpHeaders = HeadersUtil.getHeadersAuth();
    const url = environment.apiURL + '/edu/edu0203/deleteListCheck'
    return this.http.post<any>(url, request, { headers: headers });
  }

  getById(eventId: String): Observable<any> {
    const headers: HttpHeaders = HeadersUtil.getHeadersAuth();
    const url = environment.apiURL + '/edu/edu0203/getById/' + eventId;
