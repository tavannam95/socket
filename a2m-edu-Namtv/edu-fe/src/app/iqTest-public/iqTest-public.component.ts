...
@Component({
  ...
  templateUrl: './iqTest-public.component.html',
  styleUrls: ['./iqTest-public.component.css']
})
export class IqTestPublicComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(){
    ...
    this.route.queryParams.subscribe(async (params) => {
      this.iqTestId = params['id']?params['id']:"";
    });
    this.createNewUser(this.iqTestId);
  }
  ngOnDestroy() {
  ...
  getIqTestSubmit(iqTestId: any){
    var param = {
      iqTestId: iqTestId,
      userInfoId: '',
    }
    this.iqTestPublicService.getIqTestSubmit(param).subscribe((response) => {
  ...
  doSubmit(){
    ...
    this.iqTestPublicService.submit(submitRequest).subscribe({
      next: (response) =>{
        clearInterval(this.timerInterval);
