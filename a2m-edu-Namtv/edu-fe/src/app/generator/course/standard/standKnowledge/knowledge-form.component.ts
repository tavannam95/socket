...
export class KnowledgeFormComponent implements OnInit {
  ...
  @Output()
  saveKnow: EventEmitter<any> = new EventEmitter();
  @Output()
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<KnowledgeFormComponent>,
  ...
  getListStandard(){
    this.standardService.getListStandBySubjectId(this.subjectId).subscribe((response) => {
      this.listStandards = response[CommonConstant.DETAIL_KEY];this.listStandards = response[CommonConstant.DETAIL_KEY];this.standardService.getListStandHistoryBySubjectId(this.subjectId).subscribe((response) => {
