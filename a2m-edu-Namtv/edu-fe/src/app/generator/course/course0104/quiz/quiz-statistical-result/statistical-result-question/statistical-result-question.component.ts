...
@Component({
  selector: 'app-statistical-result-question',
  templateUrl: './statistical-result-question.component.html',
  styleUrls: ['./statistical-result-question.component.css']
})
export class StatisticalResultQuestionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.quizId = quizId; this.route.queryParams.subscribe(async (params) => {
  ...
  getDataChart(){
    ...
    this.quizService.getStatisResult(param).subscribe((response) => {
  ...
  getListQuestion(){
    this.pageRequest.quizId = this.quizId;
    this.quizService.getListStatisticalByQuestion(this.pageRequest).subscribe((response) => {
