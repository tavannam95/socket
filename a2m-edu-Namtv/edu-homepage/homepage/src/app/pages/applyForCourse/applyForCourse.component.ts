...
@Component({
  selector: 'app-applyForCourse',
  templateUrl: './applyForCourse.component.html',
  styleUrls: ['./applyForCourse.component.css'],
})
export class ApplyForCourseComponent implements OnInit, AfterViewInit {
  //Star Valid Form
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(async (params) => {
      ...
      this.eventId = CommonUtil.deCodeBase64( params['eventId'] ).replace(CommonConstant.ATOWM_EDU_CONSTANT, "");
  ...
  onSave(invalid: any, form: any) {
    if (invalid) {
      ...
      let dialogSpinner = this.dialog.open(SpinnerDialogComponent, {
  ...
  getEventById() {
    this.applyService.getEventById(this.eventId).subscribe((response: any) => {
      this.eventCourse = response[CommonConstant.DETAIL_KEY];
      this.courseId = this.eventCourse.courseModel.key;
      this.getCourseById();
      });
  }
  ...
  handleKeyupPhone(event: any) {
    ...
    if (!isNumber) {
      if (keyStr != null || keyStr != undefined || keyStr != '') {
        this.candidate.candidatePhone = this.candidate.candidatePhone.replace(
          keyStr,
          ''
        );
        this.candidate.candidatePhone = this.candidate.candidatePhone.replace(
