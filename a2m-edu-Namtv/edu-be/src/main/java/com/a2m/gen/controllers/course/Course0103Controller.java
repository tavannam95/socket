...
@RestController
@RequestMapping(value = "/api/course/course0103")
public class Course0103Controller {
    ...
    @PostMapping(value = "/saveOrdNo")
    @ResponseBody
    @Transactional(rollbackFor = { Exception.class })
    public Map<Object, Object> saveOrdNo(@RequestBody SbjChapterModel arg) throws Exception {
    	Map<Object, Object> res = new HashMap<Object, Object>();
    	try {
    		res = course0103Service.saveOrdNo(arg);
    	} catch (Exception e) {
			res.put(CommonContants.STATUS_KEY, CommonContants.RESULT_NG);
			res.put(CommonContants.MESSAGES_KEY, e.toString());
		}
...
//          
    ...
    @PostMapping(value = "/saveViewDoc")
    @ResponseBody
    @Transactional(rollbackFor = { Exception.class })
    public Map<Object, Object> saveViewDoc(@RequestBody DocUserMapModel arg) throws Exception {
    	Map<Object, Object> res = new HashMap<Object, Object>();
    	try {
    		res = course0103Service.saveViewDoc(arg);
    	} catch (Exception e) {
			res.put(CommonContants.STATUS_KEY, CommonContants.RESULT_NG);
			res.put(CommonContants.MESSAGES_KEY, e.toString());
		}
    ...
    public ResponseEntity<?> getInfoCourse(@PathVariable("id") Long Id) {
        Map<String, Object> response = new HashMap<>();
        try {
             Map detail = course0103Service.getInfoCourse(Id);
            response.put(CommonContants.DETAIL_KEY, detail);
        } catch (Exception e) {
            e.printStackTrace();
			response.put(CommonContants.STATUS_KEY, CommonContants.RESULT_NG);
	...
	public ResponseEntity<?> searchByTag(ParamSearchModel search) throws Exception {
		Map<String, Object> response = new HashMap<>();
		try {
		List<SbjChapterModel> lst = course0103Service.searchByTag(search);	
//		List<AemLecture> lst = subjectDao.getLectures(search);
        Long totalItems = SbjChapterDao.countChapterByTag(search);
